...
function applyAsync(func, context, args, callback) {
  var result = func.apply(context, args.concat(callback));
...
Tapable.prototype.applyPluginsAsyncSeries = Tapable.prototype.applyPluginsAsync = function applyPluginsAsync(name) {
	...
	var innerCallback = copyProperties(callback, function next(err) {
		...
		applyAsync(plugins[i], _this, args, innerCallback);
	});
  applyAsync(plugins[0], this, args, innerCallback);
};
...
Tapable.prototype.applyPluginsAsyncSeriesBailResult = function applyPluginsAsyncSeriesBailResult(name) {
	...
	var i = 0;
	var _this = this;
	var innerCallback = copyProperties(callback, function next() {
...
Tapable.prototype.applyPluginsAsyncSeriesBailResult1 = function applyPluginsAsyncSeriesBailResult1(name, param, callback) {
	...
	var innerCallback = copyProperties(callback, function next(err, result) {
		...
		applyAsync(plugins[i], _this, [param], innerCallback);
	});
	applyAsync(plugins[0], this, [param], innerCallback);
};
...
Tapable.prototype.applyPluginsAsyncWaterfall = function applyPluginsAsyncWaterfall(name, init, callback) {
	...
	var next = copyProperties(callback, function(err, value) {
		...
		applyAsync(plugins[i], _this, [value], next);
	});
	applyAsync(plugins[0], this, [init], next);
};
...
Tapable.prototype.applyPluginsParallelBailResult = function applyPluginsParallelBailResult(name) {
	...
	for(var i = 0; i < plugins.length; i++) {
		...
		applyAsync(plugins[i], this, args, innerCallback);
	}
};
...
Tapable.prototype.applyPluginsParallelBailResult1 = function applyPluginsParallelBailResult1(name, param, callback) {
	...
	for(var i = 0; i < plugins.length; i++) {
		...
		applyAsync(plugins[i], this, [param], innerCallback);
	}
};
